...
try{    
    ...
    if(count($rows) >= 1){
        ...
        createUser($username,$first_name,$last_name,$email,$hashedPwd, $db, $whereSuccess, $age, $where);
    }
   ...
   function createUser($username,$first_name,$last_name,$email,$hashedPwd, $db, $whereSuccess, $age, $where){
        try{
            $POST = [$username,$first_name,$last_name,$email,$hashedPwd,$age,1];
            $stmt = $db->prepare('INSERT INTO users (username, first_name, last_name, email, password, age, active) values(?,?,?,?,?,?,?)');
            $stmt->execute($POST);
...
<tr>
<td align="left" style="padding:0;Margin:0;padding-bottom:5px"><h2 style="Margin:0;line-height:29px;mso-line-height-rule:exactly;font-family:arial, "helvetica neue", helvetica, sans-serif;font-size:24px;font-style:normal;font-weight:normal;color:#333333">Welcome '.$first_name.' to the 1st mandatory assignment!</h2></td>
</tr>
<tr>
...
<td align="center" style="padding:0;Margin:0">
<a style="margin:0; border:unset; border-radius:2px; padding: 1rem 2rem 1rem 2rem; font-size:2rem; background-color:#4eb1cb; color:white; font-wight:bold; text-decoration: none;" href="http://localhost/signup/verify/'.bin2hex($token).'/'.$selector.'"> 
